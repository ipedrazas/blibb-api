...
def send_invites(owner, contacts):
    ...
    mail = {'from': "info@oioi.me", 'subject': subject, 'txt_body': txt_mail, 'html_body': html_mail, 'to_name': ''}
    for p in contacts:
        if is_valid_email(p):
            mail['to_address'] = p
...
def send_invitation_mail(mail):
    ...
    message = sendgrid.Message(mail['from'], mail['subject'], mail['txt_body'], mail['html_body'])
